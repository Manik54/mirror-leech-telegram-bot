...
def _search(key, site, message, tool):
    ...
    if tool == 'api':
        ...
        if total_results != 0:
            msg = f"🗒️<b>Found {min(total_results, SEARCH_LIMIT)}</b>"
...
def _getResult(search_results, key, message, tool):
        ...
        if tool == 'api':
            try:
                ...
                if "Files" in result.keys():
                    ...
                    msg += f"🗃️<b>Size: </b>{result['Size']}<br>"
                    msg += f"🛰️<b>Seeders: </b>{result['Seeders']} | 🧲<b>Leechers: </b>{result['Leechers']}<br>"
            except KeyError:
                pass
            try:
            ...
            msg += f"🗃️<b>Size: </b>{get_readable_file_size(result.fileSize)}<br>"
            msg += f"🛰️<b>Seeders: </b>{result.nbSeeders} | 🧲<b>Leechers: </b>{result.nbLeechers}<br>"
            link = result.fileUrl
    ...
    editMessage(f"📩<b>Creating</b> {len(telegraph_content)} 🗒️<b>Telegraph pages.</b>", message)
    ...
    sleep(0.5)
    if len(path) > 1:
        editMessage(f"📝<b>Editing</b> {len(telegraph_content)} 🗒️<b>Telegraph pages.</b>", message)
